...
/*
 * Routes are mounted twice on purpose. Deployed alongside the client on a
 * single domain the API lives under /api, which keeps everything same-origin
 * and removes CORS entirely; run as its own service it answers at the root.
 * Both mounts point at the same handlers.
 */
for (const base of ['/', '/api']) {
    app.use(base, interviewRoutes);
    app.use(base, jobRoutes);
    app.use(base, networkRoutes);
